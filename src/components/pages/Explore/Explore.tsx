...
const renderMobileModal = (mobileModalOpen: any, setMobileModalOpen: any) => {
    return (
        <Modal
            onClose={() => {
        ...
        >
            <Modal.Header className={"flex justify-content-center text-center"}>{Translate("Welcome")}</Modal.Header>
            <Modal.Content className={"text-center"}>{Translate("OptimisedFunctionality")}</Modal.Content>
...
const renderUnityBanner = (dispatch: any, setShowUnityBanner: any) => {
    return (
        <div
            className={"px-4 flex center-item justify-content-between w-100 visible"}
            style={{ backgroundColor: "#E8EBEF", height: "50px" }}
        >
            <div style={{ width: "80%" }}>
                {Translate("UnityBanner", ["Currently"])}
                <a href={"https://www.who.int/emergencies/diseases/novel-coronavirus-2019/technical-guidance/early-investigations"} target="_blank" rel="noopener noreferrer">
                    {Translate("UnityBanner", ["ProtocolLink"])}
                </a>{" "}
                {Translate("UnityBanner", ["ViewDatabase"])}{" "}
                <span
                    className={"link"}
                    onClick={() => {
                ...
                >
                    {" "}
                    {Translate("UnityBanner", ["WHOUnityFilter"])}{" "}
                </span>
...
const renderMobileUnityBanner = (dispatch: any, setShowUnityBanner: any) => {
    return (
        <div
            className={"px-4 flex center-item justify-content-between w-100"}
...
export default function Explore({ initialFilters }: ExploreProps) {
    ...
    return (
        <>
            {renderMobileModal(mobileModalOpen, setMobileModalOpen)}
            <div className="explore__dashboard">
                {!isMobileDeviceOrTablet ? (
                    <>
                        {showUnityBanner && renderUnityBanner(dispatch, setShowUnityBanner)}
                        <div className="flex w-100 explore__container">
                    ...
                    <div className="flex w-100">
                        {showUnityBanner && renderMobileUnityBanner(dispatch, setShowUnityBanner)}
                        <ExploreMobile />
                    </div>
                )}
            </div>
